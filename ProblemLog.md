...
## v7.0-fix3 我的页面头像重构 + 火花页Tab样式统一 (2026-05-05)

### 需求清单

| # | 需求 | 文件 |
|---|------|------|
| 1 | **我的页面头像**：白底、64px、左对齐 | `profile/page.tsx` |
| 2 | **头像+用户名间距**：gap-4（16px）| `profile/page.tsx` |
| 3 | **删除计数区块**：火花/故事/匹配数字移除 | `profile/page.tsx` |
| 4 | **火花页Tab**：改为故事页同款底部边框样式 | `library/page.tsx` |
| 5 | **移除圆圈按钮**：Tab 不再使用 rounded-full | `library/page.tsx` |
...
| 项目 | 旧 | 新 |
|------|-----|-----|
| 背景色 | 彩色（根据首字母动态） | 白底 `bg-white/10` |
| 文字色 | 白色 | 灰色 `text-white/50` |
| 尺寸 | 48px | 64px |
| 边框 | `border-[#e2b04a]/20` | `border-white/10` |
| 布局 | 整体居中 | 左对齐 `px-2` |
| 间距 | gap-3（12px） | gap-4（16px） |

**统计数据**：火花/故事/匹配三个计数数字及对应的 `loadStats` 逻辑全部移除

### 火花页 Tab 变更

**旧样式（圆角按钮）**：
```
rounded-full px-4 py-2 bg-[#e2b04a]/15 border border-[#e2b04a]/25
```

**新样式（故事页同款底部边框）**：
```
pb-1 border-b-2 text-[#e2b04a] border-[#e2b04a]
```
...
| 文件 | 变更 |
|------|------|
| `src/app/profile/page.tsx` | 头像白底64px左对齐、删计数、gap-4 |
| `src/app/library/page.tsx` | Tab 改为故事页底部边框样式 |
| `docs/qunxiangxinhuo-TDD-v7.0.md` | 更新我的页面和火花页描述 |
| `IMPORTANT.md` | 新增布局规范和Tab样式规范 |
...
## v7.0-fix5 P0登录Bug紧急修复：关闭浏览器后强制重新登录 + 退出后单次点击登录 (2026-05-05)

### 问题1（P0）：未登录时保留痕迹直接进入发现页

**现象**：用户关闭浏览器后重新打开网站，不需要重新登录就直接进入发现页

**根因链**：
```
用户登录成功
  → next-auth 设置 JWT session cookie
  → cookie 默认 maxAge = session.maxAge = 24小时
  → 这是一个持久化 cookie（Persistent Cookie）
  → 用户关闭浏览器
  → cookie 仍然保留在浏览器中
  → 用户重新打开网站
  → middleware.ts 的 getToken 读取到有效 JWT token
  → 判定用户已登录
  → 直接允许访问 /home 等页面
  → 用户期望关闭浏览器后需重新登录
```

**根因1**：NextAuth v4 默认将 JWT cookie 设为持久化 cookie（maxAge = session.maxAge），关闭浏览器后 cookie 不会自动清除
**根因2**：`middleware.ts` 的 `getToken` 只验证 token 签名和过期时间，不区分 cookie 是 session cookie 还是 persistent cookie
**根因3**：`LoginForm.tsx` 只在 `status === 'unauthenticated'` 时清理 localStorage，但关闭浏览器后再打开时 `status` 初始是 `loading`，不会触发清理

**修复方案**：
1. **`src/lib/auth.ts` 添加 `cookies` 配置**：显式覆盖 sessionToken cookie 选项，**不设置 maxAge**，使其成为 session cookie（关闭浏览器即自动清除）
   - JWT 的 24 小时服务端有效期仍然保留
   - 登录后浏览器保持打开时，24 小时内无需重新登录
   - 关闭浏览器后再打开，cookie 自动清除，必须重新登录
2. **`src/app/LoginForm.tsx` 挂载时无条件清理**：组件 `useEffect` 挂载时无条件清除 `xh_user`/`xh_identity`/`xh_user_id` + `sessionStorage.clear()`，防止任何形式的本地残留
3. **`src/components/layout/AppShell.tsx` 无条件清除**：`sessionStatus === 'unauthenticated'` 时**无条件**清除 localStorage（移除之前 `if (localUser || localUserId)` 的条件判断）

### 问题2（P0）：退出登录后需点击两次才能进入发现页

**现象**：用户点击"退出登录"后回到登录页，输入用户名密码点击登录，页面没有反应；需再次点击才能成功进入发现页

**根因链**：
```
用户点击退出登录
  → profile/page.tsx: signOut({ redirect: false }) + router.push('/') + router.refresh()
  → 用户到达登录页
  → 输入用户名密码，点击登录按钮
  → LoginForm handleLogin 执行 signIn('credentials', ...)
  → signIn 成功，status 从 loading→authenticated
  → 触发 LoginForm useEffect: status === 'authenticated' → router.replace('/home')
  → handleLogin 继续执行: router.push('/home')
  → router.replace + router.push 几乎同时执行
  → Next.js 路由系统处理冲突
  → 导航失败，用户仍停留在登录页
  → 用户再次点击登录按钮
  → 此时 status 已经是 authenticated，useEffect 不会触发
  → 只有 router.push('/home') 执行
  → 导航成功
```

**根因1**：`LoginForm.tsx` 中 `handleLogin` 的 `router.push('/home')` 与 `useEffect` 中 `router.replace('/home')` 在 `signIn` 成功后**几乎同时执行**，产生路由冲突
**根因2**：`profile/page.tsx` 退出后使用 `router.push('/')` + `router.refresh()` 是软刷新，next-auth 的客户端 session 缓存可能未完全重置

**修复方案**：
1. **`src/app/LoginForm.tsx` 移除冲突 useEffect**：删除 `status === 'authenticated'` 时的 `router.replace('/home')` useEffect，避免与 `handleLogin` 中的 `router.push` 冲突
   - 已登录用户的重定向由 `middleware.ts`（服务端）和 `AppShell.tsx`（客户端）统一处理
2. **`src/app/LoginForm.tsx` 移除 `router.refresh()`**：`handleLogin` 末尾只保留 `router.push('/home')`，不再调用 `router.refresh()`
3. **`src/app/profile/page.tsx` 退出改为硬刷新**：将 `router.push('/')` + `router.refresh()` 改为 `window.location.href = '/'`
   - 硬刷新彻底卸载 React 应用，重置所有客户端状态
   - `SessionProvider` 重新初始化，`useSession` 从头开始获取
   - 确保退出后登录页的 session 状态完全干净

### 额外修复

**`src/app/profile/page.tsx` "去登录"按钮跳转 `/login`**：项目无 `/login` 路由（v7.0-fix4 已删除），改为 `router.push('/')`

### 4轮自检结果

| 轮次 | 检查项 | 结果 |
|------|--------|------|
| 第1轮 | 代码审查：确认所有修改点正确 | ✅ 通过 |
| 第2轮 | TypeScript 编译检查 | ✅ 通过（零错误） |
| 第3轮 | 排查遗漏：`router.refresh()` 全部清除 | ✅ 通过 |
| 第4轮 | Next.js 构建：65/65 路由通过 | ✅ 通过 |
...
| 文件 | 变更 |
|------|------|
| `src/lib/auth.ts` | 新增 `cookies` 配置，sessionToken 设为 session cookie（关闭浏览器即失效） |
| `src/app/LoginForm.tsx` | 挂载时无条件清理残留；移除 `status === 'authenticated'` useEffect；移除 `router.refresh()` |
| `src/app/profile/page.tsx` | 退出改为 `window.location.href = '/'` 硬刷新；"去登录"跳转 `/` |
| `src/components/layout/AppShell.tsx` | `unauthenticated` 时无条件清除 localStorage |

### 预防措施

1. **认证导航唯一性原则**：登录成功后的导航只能有一个来源，要么 `useEffect` 监听 session 状态，要么 `handleLogin` 手动导航，不可两者并存
2. **退出后硬刷新原则**：涉及认证状态重置的操作（退出登录），优先使用 `window.location.href` 硬刷新，彻底重置所有客户端状态
3. **cookie 策略显式声明**：NextAuth 的 cookie 配置必须显式声明，不能依赖默认行为。需要关闭浏览器后失效的，必须不设置 `maxAge`
4. **无条件清理原则**：任何检测到未认证状态的逻辑，都应无条件清除所有本地认证相关数据，不能用 `if (exists)` 做条件判断
