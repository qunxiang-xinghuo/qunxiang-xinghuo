...
for path in ['/home', '/story-hall', '/']:
    out, err = ssh_cmd(client, f"curl -s http://81.70.59.228:3000{path} | head -c 500")
    print(f"\n{path}:")
    print(out[:300])
...
# curl第一个文件
out, err = ssh_cmd(client, f"JS=$(ls {DEPLOY_DIR}/.next/static/chunks/*.js | head -1 | sed 's|{DEPLOY_DIR}/.next/static/chunks/||') && echo \"Testing: $JS\" && curl -sI http://81.70.59.228:3000/_next/static/chunks/$JS | head -5")
print(f"\n第一个JS文件curl结果:\n{out}")

# 测试已知文件名的CSS
out, err = ssh_cmd(client, "curl -sI http://81.70.59.228:3000/_next/static/css/ | head -5 || echo 'no css dir'")
print(f"CSS目录:\n{out}")
...
# 4. PM2日志
print("\n\n【4】PM2日志（最近20行）")
out, err = ssh_cmd(client, "pm2 logs qunxiang-xinghuo --lines 20 --nostream 2>/dev/null || cat /root/.pm2/logs/qunxiang-xinghuo-out.log | tail -20")
print(out)

print("\n\n【5】PM2错误日志（最近20行）")
out, err = ssh_cmd(client, "cat /root/.pm2/logs/qunxiang-xinghuo-error.log | tail -20")
print(out or "(无错误日志)")
