...
[{ "title": "脑洞标题", "scenario": "情境描述", "category": "general" }, ...]`;
    ...
    return items.filter((item: any) => item.title && item.scenario).map((item: any, index: number) => ({
      id: `ds-${Date.now()}-${index}`, title: String(item.title).slice(0, 60), scenario: String(item.scenario).slice(0, 300),
...
async function fetchZhihuHotBrainholes(): Promise<BubbleBrainhole[]> {
  try {
    ...
    return result.Data.Items.map((item, index) => ({
      id: `zh-hot-${Date.now()}-${index}`, title: item.Title.slice(0, 60),
  ...
  for (let i = 0; i < Math.min(queries.length, 3); i++) {
    try {
      ...
      result.Data.Items.forEach((item, idx) => {
        brainholes.push({
          id: `zh-search-${Date.now()}-${i}-${idx}`, title: item.Title.slice(0, 60),
...
function generateFallbackBrainholes(count: number): BubbleBrainhole[] {
  const fallbackData = [
    { title: "急诊室里的道德困境", scenario: "凌晨2点，一位急诊科医生面对两个同时送达的病人：一个是酒驾肇事者，一个是被他撞伤的行人。血库只剩下最后一袋匹配型血，而两个病人都需要立即输血...", category: "medical" },
    { title: "裁员名单上的秘密", scenario: "公司年会前夜，HR总监发现裁员名单上有自己最好的朋友。更意外的是，朋友在名单确认栏签了字，似乎早已知情...", category: "workplace" },
    { title: "学区房背后的交易", scenario: "为了孩子的入学资格，一对夫妻决定假离婚。当一切办妥后，丈夫却有了新的恋情，妻子面临人财两空...", category: "life" },
    ...
    { title: "创业者的对赌协议", scenario: "创业者与投资人签了对赌协议。眼看deadline临近，他发现唯一能完成对赌的方式，是牺牲团队里一名核心成员的利益...", category: "workplace" },
    { title: "全职妈妈的秘密", scenario: "一位全职妈妈在家长群里偶然发现，丈夫与另一位家长的聊天记录异常亲密。更令她震惊的是，那位家长的身份...", category: "life" },
    { title: "酒吧老板的客人", scenario: "酒吧老板发现一位常客每晚都在等一个不会出现的人。三个月后，那个人真的出现了，但身份让老板措手不及...", category: "life" },
    { title: "民宿里的命案", scenario: "民宿老板发现客人在房间里离世。警方调查时，老板突然想起三天前另一位客人曾提到过这个房间不干净...", category: "life" },
    ...
    { title: "北漂程序员的抉择", scenario: "程序员终于攒够首付，却发现房东要涨租30%。更糟的是，他的roommate偷偷用他的名义贷了款...", category: "life" },
    ...
    { title: "急诊护士的发现", scenario: "急诊护士发现一位交通事故受害者的伤情与描述不符。深入调查后，她揭开了一场精心策划的骗保骗局...", category: "medical" },
  ...
  return fallbackData.slice(0, count).map((item, index) => ({
    id: `fb-${Date.now()}-${index}`, title: item.title, scenario: item.scenario,
    hotScore: 45 + Math.floor(Math.random() * 30), category: item.category, source: "fallback", difficulty: "medium",
    reactionCount: Math.floor(Math.random() * 30), sparkCount: Math.floor(Math.random() * 10), collectionCount: Math.floor(Math.random() * 5),
  }));
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    // FIX: limit参数计算修复 - 原来是 Math.min(Math.max(..., 30), 1) 永远返回1
    ...
    if (!refresh) {
      try {
        ...
        const cached = await db.brainhole.findMany({
          where: { status: "approved", createdAt: { gte: oneHourAgo } },
          orderBy: { hotScore: "desc" },
          take: limit,
        });
        if (cached.length >= Math.min(limit, 10)) {
          return NextResponse.json(apiResponse({ brainholes: cached, total: cached.length, source: "cache" }));
        }
      } catch (dbErr) { console.error("[Bubble] Cache read failed:", dbErr); }
    }

    // 2. 从三渠道聚合数据（并行请求，任一失败不影响其他）
    ...
    // 4. 如果所有API都失败，用fallback保底
    const apiCount = zhihuHot.length + zhihuSearch.length + deepseekBrainholes.length;
    const fallbackCount = Math.max(0, limit - apiCount);
    const fallback = generateFallbackBrainholes(Math.max(fallbackCount, limit));

    // 5. 合并去重
    const merged = mergeAndDeduplicate(zhihuHot, zhihuSearch, deepseekBrainholes, fallback);
    const final = merged.slice(0, limit);

    console.log(`[Bubble] Generated ${final.length} bubbles: zhihuHot=${zhihuHot.length}, search=${zhihuSearch.length}, deepseek=${deepseekBrainholes.length}, fallback=${fallback.length}`);
    ...
    try {
      for (const item of final) {
        await db.brainhole.upsert({
          where: { id: item.id },
          update: { hotScore: item.hotScore, reactionCount: item.reactionCount, sparkCount: item.sparkCount, collectionCount: item.collectionCount, updatedAt: new Date() },
          create: { ...item, status: "approved", recencyBoost: true },
        });
      }
    ...
    const fallback = generateFallbackBrainholes(20);
    return NextResponse.json(apiResponse({ brainholes: fallback, total: fallback.length, source: "emergency_fallback" }));
  }
}
