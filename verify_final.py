...
print("\n=== 3. 静态JS资源验证 ===")
stdin, stdout, stderr = client.exec_command('find /path/to/remote/project/.next/static -name "*.js" | head -1')
js_file = stdout.read().decode().strip()
if js_file:
    url_path = js_file.replace('/path/to/remote/project/.next/static/', '/_next/static/')
    stdin, stdout, stderr = client.exec_command(f"curl -sI http://localhost:3000{url_path} | head -2")
    print(f"{url_path}:")
    print(stdout.read().decode().strip())

print("\n=== 4. 静态CSS资源验证 ===")
stdin, stdout, stderr = client.exec_command('find /path/to/remote/project/.next/static -name "*.css" | head -1')
css_file = stdout.read().decode().strip()
if css_file:
    url_path = css_file.replace('/path/to/remote/project/.next/static/', '/_next/static/')
    stdin, stdout, stderr = client.exec_command(f"curl -sI http://localhost:3000{url_path} | head -2")
    print(f"{url_path}:")
    print(stdout.read().decode().strip())
else:
