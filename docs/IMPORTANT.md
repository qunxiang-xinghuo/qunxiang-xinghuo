...
| 任务 | 文件 | 说明 |
|------|------|------|
| 状态切换定义 | `src/lib/ai/personas.ts` | 全部12角色注入"对话/任务/审核"三种状态规则 |
| 状态上下文注入 | `src/app/api/ai/chat/route.ts` | 支持 `state` 参数注入 `[STATE:xxx]` 标记 |
| 系统提示词更新 | `src/lib/ai/personas.ts` | companion 增加任务拆解示例，全角色增加状态判断规则 |

### 核心机制

1. **三种状态定义**：
   - 对话状态：用户只是聊天 → AI 像真人一样自由回应
   - 任务状态：用户提出可操作需求 → 内部拆解为操作清单逐步执行
   - 审核状态：用户结束对白 → AI 引导用户点击 🏁 谢幕按钮

2. **状态判断**：AI 根据用户消息内容 + `[STATE:xxx]` 系统标记自行判断

3. **任务拆解规范**（companion）：
   - 用户想玩故事 → `search_stories` → 展示结果 → 引导用户选择
   - 用户想匹配人 → `find_online_user` → 找到→创建真人房间，没找到→创建AI房间
   - 用户说"随便" → `search_stories(random)` 推荐一个
   - 用户说"开始" → `create_room` 直接创建AI房间

4. **构建验证**：`npm run build` 81/81 页 ✅
5. **Git 推送**：已推送 `dev` 分支

---

## v9.1 故事系统"好玩化"改造 — 部署教程
...
| 任务 | 文件 | 说明 |
|------|------|------|
| 故事大厅悬念化 | `story-hall/page.tsx` | 卡片以悬念开场(summary 3行)、时代氛围置顶、金色左边框、🔍开始探索CTA、文案冒险化 |
| 故事详情代入感 | `story/[id]/page.tsx` | 移除起承转合标签(改为氛围化故事背景)、角色emoji图标、黄色选中边框、选角反馈、🎲交给命运、弹窗文案戏剧化 |
| 对白室沉浸感 | `room/[id]/page.tsx` | 标题白色+时代斜体、帷幕已拉开空状态、火花badge金色、谢幕/揭开真相文案、真相浮现阶段标签(开场/发展/转折/真相)、✏️创作引导按钮 |
| 我的故事成就感 | `my-stories/page.tsx` | 标签改为⚔️我的冒险/🎬我的故事坑、创建按钮文案趣味化、空状态引导冒险化 |
| 火花详情故事感 | `spark-detail/SparkDetailClient.tsx` | 头部显示分类、火花消息黄色左边框凸显、✏️基于这段对白创作引导 |

### 设计原则（游戏化）
- 🎭 **戏剧化文案**：用"帷幕""谢幕""冒险""真相"替代功能性术语
- 🎲 **悬念驱动**：卡片以故事悬念句为核心内容，第一眼就被勾住
- 👤 **角色代入**：emoji图标+黄色选中反馈+"你选择了XX"过渡文案
- ✨ **仪式感**：匹配成功"命运让你们相遇"、揭晓真相渐入动画
- ✏️ **创作闭环**：每个结束页面都有"基于这个故事写一个"的引导
