...
1. **登录页显示修复**：退出登录后真正清除 session，确保用户能看到登录页
2. **页面标题统一居中**：发现/火花/故事/我的所有页面标题统一居中
3. **火花页上移**：减少顶部 padding，内容更紧凑
4. **登录页泡泡装饰**：保留 7 个透明上升泡泡
...
- **Tab 切换**：最新火花 / 最热火花（故事页样式：底部边框+文字高亮，非圆圈按钮）
...
### 3.5 我的页（/profile）
- **布局**：头像（64px白底圆形）左 + 用户名右，左对齐
- 头像和用户名之间有间距（gap-4）
- 无统计数据计数（已移除火花/故事/匹配数字）
- 功能菜单：
  - 我的收益
  - 个人疗愈
