...
*文档版本：v6.0-fix2 泡泡彻底修复版 | 已部署 | 2026-05-02*
...
**根因链：**
```
泡泡API: id = `ds-${Date.now()}-${index}` （每次请求都不同）
  → 同一标题在数据库中有多条重复记录，id各不相同
  → 用户点击泡泡A（id=ds-123-0）
  → 跳转 /brainhole/ds-123-0
  → useBrainhole hook 只加载一次 /api/brainholes
  → hook列表中只有 ds-456-0（同标题不同id）
  → getBrainholeById("ds-123-0") 返回 undefined
  → 显示"脑洞不存在"
```

**修复：**
1. fallback数据使用预定义稳定id
2. 新生成数据先查数据库是否有同标题记录
3. 有则返回已有id，无则 `prisma.create()` 生成cuid()
4. 查询时排除旧临时id格式

### 为什么收藏的脑洞找不到

**根因：** `/api/brainholes/collected` 接口存在，但素材库 `/library` 只显示「我的素材」和「广场素材」，没有「我的收藏」标签页。

**修复：** 素材库增加第三个标签页「我的收藏」，调用 `/api/brainholes/collected` 展示收藏的脑洞。
