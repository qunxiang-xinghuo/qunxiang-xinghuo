...
### 12.6 v4.1 首页+双人模式核心流程重构（2026-04-29）

#### 需求背景
用户要求重构首页和双人模式核心流程，简化入口，强化匹配体验。
...
**三、身份选择（复用DuoIdentityModal）**：
- 知乎身份（已认证职业身份）
- AI随机生成（15个预设角色随机分配）
- 自定义角色（用户输入）

**四、匹配等待页重构（核心）**：
- 刘看山形象动画 + "刘看山正在为你寻找对撞人…"
- **60秒倒计时**：大字体数字 + 进度条，每秒更新
- 匹配成功 → 自动跳转 `/room/:id`

**五、超时弹窗状态机（两次超时）**：

```
[matching] --60s超时--> [timeout弹窗]
  |
  ├─ 选"是" → 刘看山AI房间
  └─ 选"否" → [matching_2nd] --60s超时--> [timeout弹窗]
                                      |
                                      ├─ 选"是" → 刘看山AI房间
                                      └─ 选"否" → 返回首页
```

- 弹窗文案："当前暂无真人，是否与刘看山一起探讨？"
- 按钮：【是，与刘看山对白】【否，继续等待/返回首页】

#### 修改文件清单

| 文件 | 修改内容 |
|------|----------|
| `src/app/page.tsx` | 简化首页，只保留泡泡+双人模式入口 |
| `src/app/duo-match/page.tsx` | 移除脑洞选择，直接进入身份选择 |
| `src/app/duo-waiting/page.tsx` | 两次超时弹窗状态机重构 |
| `src/components/match/DuoIdentityModal.tsx` | 适配快速匹配文案 |

#### 本地验证
- `npm run build` 通过
- dev server: `http://localhost:3000`
- 首页 `/` → 200 OK
- 双人匹配 `/duo-match` → 200 OK（直接进入身份选择）
- 等待页 `/duo-waiting` → 200 OK
- 泡泡API → 200 OK，31个泡泡

---

### 12.7 v4.1-fix 首页恢复+登录检查+泡泡交互（2026-04-29）

#### 问题诊断

**问题1：双人模式"蹦错误"**
- 根因：用户未登录时调用 `POST /api/match` → 401 未授权
- 前端显示 `alert('匹配请求失败')`，用户体验差
- **修复**：`duo-match/page.tsx` 添加登录状态检查
  - 进入页面时 `fetch('/api/auth/session')` 检查登录状态
  - 未登录 → `alert('请先登录后再进行双人匹配')` → 自动跳转 `/login`
  - 已登录 → 正常显示身份选择弹窗

**问题2：请求失败后弹窗消失**
- 根因：`handleConfirmIdentity` 中 `setShowIdentityModal(false)` 后如果请求失败，不重新显示弹窗
- **修复**：所有错误分支（401/500/网络错误）都 `setShowIdentityModal(true)` 重新显示身份选择

**问题3：首页模式卡片被移除**
- 根因：v4.1 改造时过度简化，只保留了双人模式入口
- **修复**：恢复原有模式卡片布局（发现/双人/多人/连载）

**问题4：泡泡无鼠标交互**
- **修复**：`Bubble.tsx` 添加
  - `onMouseEnter` → `scale(1.15)` 放大 + 发光阴影
  - `onMouseMove` → 视差偏移（泡泡跟随鼠标方向轻微移动）
  - `onMouseLeave` → 恢复原位
