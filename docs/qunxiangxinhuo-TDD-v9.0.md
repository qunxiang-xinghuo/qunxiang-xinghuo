...
### v9.3-emergency 已修复（2026-04-29）

| 问题 | 状态 | 描述 | 修复 |
|------|------|------|------|
| 🔴 火花页为空 | **已修复** | `Asset.isPublic` 默认 `false`，Top3 查询返回空数组；详情页 `notFound()` 返回空白 | Top3 兜底查询全部 Asset；详情页显示 `"对白记录正在整理中"` |
| 🔴 人机创建失败 | **已修复** | 卡死 AI 房间阻塞新创建；创建流程复杂易失败；错误 500 无具体信息 | 暴力清理所有 active ai_duet；无脑新建强制 `status=active`；打印完整错误到日志 |
