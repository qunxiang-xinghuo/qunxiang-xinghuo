...
| 状态 | 进入信号 | 行为模式 | 退出信号 |
|------|---------|---------|---------|
| **对话状态** | 用户闲聊/分享感受/问开放性问题 | 自由回应，字数/语气灵活 | 用户提出可操作需求 |
| **任务状态** | 用户提出可拆解成具体操作的需求 | 拆解→执行→展示结果 | 任务完成或用户切换话题 |
| **审核状态** | 用户说"结束"或点击结束按钮 | reviewer 只输出通过/不通过；其他角色引导用户点击 🏁 谢幕 | 审核完成 |
...
**API 支持**：`POST /api/ai/chat` 支持 `state` 参数，后端注入 `[STATE:对话]` / `[STATE:任务]` / `[STATE:审核]` 标记。
